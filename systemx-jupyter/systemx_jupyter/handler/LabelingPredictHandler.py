...
_LABELING_FACTORIES = {
    "hgt": HGTLabelingPipelineFactory,
    "mlp": MLPLabelingPipelineFactory,
    "xgboost": XGBoostLabelingPipelineFactory,
}

class LabelingPredictHandler(SystemXBaseHandler):
    @property
    ...
    def create_pipeline(self, input_data: dict) -> LabelingPipeline:
        ...
        self.log.info(
            "LabelingPredictHandler: nnBackend=%r featurePreset=%r modelVariant=%r", backend, preset, variant
        )

        if backend == "vamsa_static":
            return VamsaLabelingPipelineFactory.create(input_data)

        if variant:
            family = variant.split("_", 1)[0]
            if family not in _LABELING_FACTORIES:
                labeler = self.resolve_learned_labeler(backend, preset, variant_key=variant)
                return _LABELING_FACTORIES[backend].create(input_data, labeler)
            labeler = self.resolve_learned_labeler(family, preset, variant_key=variant)
            return _LABELING_FACTORIES[family].create(input_data, labeler)
    ...
    def _repair_fallback_ids(self, elements: list[dict], cells: list[dict]) -> list[dict]:
        ...
        ordered_actual_ids = [
            cell.get("cell_id") or cell.get("id") for cell in cells if (cell.get("cell_id") or cell.get("id"))
        ]
