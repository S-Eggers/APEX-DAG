...
class TestEmbedding(unittest.TestCase):
    ...
    def test_embed_fasttext(self, mock_load_model):
        ...
        self.assertTrue(
            torch.equal(result, torch.tensor([0.1, 0.2, 0.3], dtype=torch.float32))
        )
    ...
    def test_embed_gemini_standard(self, mock_genai_client):
        ...
        mock_model.models.embed_content.return_value = MagicMock(
            embeddings=[0.4, 0.5, 0.6]
        )  # Note: 'embeddings' not 'embedding'

        embedding = Embedding(EmbeddingType.GEMINI_STANDARD, self.mock_logger)
        result = embedding.embed("test sequence")

        mock_model.models.embed_content.assert_called_once_with(
            model="gemini-embedding-001",
            contents="test sequence",
            config=types.EmbedContentConfig(task_type="CLASSIFICATION"),
        )
        self.assertIsInstance(result, torch.Tensor)
        self.assertTrue(
            torch.equal(result, torch.tensor([0.4, 0.5, 0.6], dtype=torch.float32))
        )
    ...
    def test_embed_gemini_code(self, mock_genai_client):
        ...
        self.assertIsInstance(result, torch.Tensor)
        self.assertTrue(
            torch.equal(result, torch.tensor([0.7, 0.8, 0.9], dtype=torch.float32))
        )
