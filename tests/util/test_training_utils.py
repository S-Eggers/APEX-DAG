...
class TestTrainingUtils(unittest.TestCase):
    ...
    def test_constants(self):
        ...
        self.assertEqual(TASKS_PER_GRAPH_TRANSFORM_MODE_FINETUNE["REVERSED"], ["node_classification"])
        self.assertEqual(TASKS_PER_GRAPH_TRANSFORM_MODE_PRETRAIN["ORIGINAL"], ["node_classification", "edge_classification", "edge_existence"])
    ...
    def test_insufficient_negative_edges_exception(self):
        with self.assertRaisesRegex(InsufficientNegativeEdgesException, "Test message"): # Use Regex for message matching
    ...
    def test_insufficient_positive_edges_exception(self):
        with self.assertRaisesRegex(InsufficientPositiveEdgesException, "Another message"): # Use Regex for message matching
    ...
    def test_set_seed(self, mock_cudnn_benchmark, mock_cudnn_deterministic, mock_cuda_manual_seed_all, mock_manual_seed, mock_np_random_seed, mock_random_seed, mock_os_environ_setitem):
        seed_value = 42
        set_seed(seed_value)

        mock_os_environ_setitem.assert_called_once_with("PYTHONHASHSEED", str(seed_value))
