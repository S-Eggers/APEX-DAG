...
class TestNotebookStatMiner(unittest.TestCase):
    ...
    @patch('ast.parse')
    @patch('ApexDAG.sca.import_visitor.ImportVisitor')
    def test_mine_cell_data(self, MockImportVisitor, mock_ast_parse, mock_process_results):
        self.mock_notebook.__iter__.return_value = iter([([{"code": "cell1_code"}],), ([{"code": "cell2_code"}],)])
        mock_visitor_instance = MockImportVisitor.return_value
        ...
        mock_ast_parse.assert_has_calls([
            call("cell1_code"),
            call("cell2_code")
        ])
    ...
    def test_mine(self, mock_mine_import_usages, mock_mine_cell_data):
        ...
        imports, usage, counts, classes, functions, cells_with_imports = self.miner.mine(greedy=True)
