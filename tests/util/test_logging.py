...
class TestLogging(unittest.TestCase):
    ...
    def test_setup_wandb_no_name(self, mock_load_dotenv, mock_getenv, mock_wandb_init):
        ...
        mock_wandb_init.assert_called_once_with(
            project="test_project", entity="test_user"
        )
    ...
    @patch("os.getenv", return_value="test_user")
    @patch("ApexDAG.util.logging.load_dotenv")
    def test_setup_wandb_with_name(
        self, mock_load_dotenv, mock_getenv, mock_wandb_init
    ):
        setup_wandb("test_project", name="my-run-name")
        mock_wandb_init.assert_called_once_with(
            project="test_project", entity="test_user", name="hash_my_run_name"
        )
    ...
    @patch("logging.Formatter")
    @patch("logging.getLogger")
    def test_setup_logging_existing_handlers(
        self, mock_get_logger, mock_formatter, mock_stream_handler
    ):
        mock_logger = MagicMock(spec=logging.Logger)
