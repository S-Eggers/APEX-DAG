...
from ApexDAG.util.networkx_to_pyc import (
    write_dict_to_file,
    load_dict_from_file,
    split_code,
    networkx_to_pyc,
...
class TestNetworkxToPyc(unittest.TestCase):
    def test_write_dict_to_file(self):
        ...
        with patch("builtins.open", mock_file):
            ...
            mock_file().write.assert_has_calls([call("a: 1\n"), call("b: 2\n")])
    ...
    @patch(
        "torch.tensor", side_effect=lambda x: x
    )  # Mock torch.tensor to return input directly
    def test_networkx_to_pyc(
        self,
        mock_torch_tensor,
        mock_write_dict_to_file,
        mock_from_networkx,
        mock_embedding,
    ):
        g = nx.Graph()
        ...
        mock_embedding.assert_called_once_with(
            4, 256
        )  # 4 unique words: node0_label, node0_code, node1_label, node1_code
        mock_write_dict_to_file.assert_called_once_with(
            "output/word2idx.txt",
            {"node0_label": 0, "node0_code": 1, "node1_label": 2, "node1_code": 3},
        )
    ...
    def test_node2vec_embedding(self, mock_node2vec, mock_is_available):
        ...
        mock_model_instance.loader.assert_called_once_with(
