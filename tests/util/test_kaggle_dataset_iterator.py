...
class TestKaggleDatasetIterator(unittest.TestCase):
    ...
    def test_iter(self, mock_tqdm, mock_process_folders):
        mock_process_folders.return_value = (
            None  # _process_folders modifies self.results
        )
        ...
        mock_tqdm.assert_called_once_with(
            self.iterator.results, desc="Processed competitions"
        )
    ...
    @patch(
        "os.listdir", side_effect=[["subfolder1"], ["file1.json", "notebook1.ipynb"]]
    )
    ...
    def test_process_folders(
        self, mock_json_load, mock_open_file, mock_isdir, mock_join, mock_listdir
    ):
        ...
        mock_listdir.assert_has_calls(
            [call("/mock/main_folder"), call("/mock/main_folder/subfolder1")]
        )
        mock_isdir.assert_called_once_with("/mock/main_folder/subfolder1")
        mock_open_file.assert_called_once_with(
            "/mock/main_folder/subfolder1/file1.json", "r", encoding="utf-8"
        )
