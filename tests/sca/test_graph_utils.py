...
import os
from ApexDAG.sca.graph_utils import convert_multidigraph_to_digraph, get_subgraph, get_all_subgraphs, debug_graph, save_graph, load_graph
...
class TestGraphUtils(unittest.TestCase):
    ...
    def test_get_subgraph_exists(self):
        ...
        self.assertEqual(subgraph.nodes(), nx.DiGraph([('d', 'a'), ('a', 'b'), ('b', 'c')]).nodes())
        self.assertEqual(subgraph.edges(), nx.DiGraph([('d', 'a'), ('a', 'b'), ('b', 'c')]).edges())
    ...
    @patch('ApexDAG.sca.graph_utils.save_graph')
    @patch('os.path.exists')
    def test_debug_graph_prev_not_exists_save_prev_true(self, mock_os_path_exists, mock_save_graph, mock_load_graph):
    ...
    @patch('ApexDAG.sca.graph_utils.save_graph')
    @patch('os.path.exists')
    def test_debug_graph_prev_not_exists_save_prev_false(self, mock_os_path_exists, mock_save_graph, mock_load_graph):
        mock_os_path_exists.return_value = False
        G = nx.DiGraph()
    ...
    def test_load_graph_missing_edge_attributes(self, mock_os_path_exists, mock_read_gml):
        ...
        with self.assertRaises(ValueError) as cm:
            load_graph("test.gml")
        self.assertIn("Edge a -> b is missing required attributes 'code' or 'edge_type", str(cm.exception))
