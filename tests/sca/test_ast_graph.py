...
class TestASTGraph(unittest.TestCase):
    ...
    @patch('tqdm.tqdm', side_effect=lambda iterable, *args, **kwargs: iterable) # Mock tqdm to return iterable directly
    def test_get_t2t_paths_calculate(self, mock_tqdm, mock_all_simple_paths, mock_check_graph_status):
        self.ast_graph._G.add_edges_from([('a', 'b'), ('b', 'c'), ('d', 'e')])
        # 'c' and 'e' are leaf nodes
        mock_all_simple_paths.side_effect = [iter([['c', 'b', 'a', 'd', 'e']]), iter([])]
