...
class TestState(unittest.TestCase):
    ...
    def test_add_node(self):
        ...
        self.assertEqual(
            self.state._G.nodes["new_node"]["node_type"], NODE_TYPES["VARIABLE"]
        )

        self.state.add_node("new_node", NODE_TYPES["VARIABLE"])  # Add existing node
        self.mock_logger.debug.assert_called_with(
            "Node %s already exists in the graph", "new_node"
        )
    ...
    def test_node_degree(self):
        ...
        self.assertEqual(
            degrees["in"], 1
        )  # out_degree in networkx is in_degree for MultiDiGraph
        self.assertEqual(
            degrees["out"], 2
        )  # in_degree in networkx is out_degree for MultiDiGraph
    ...
    def test_add_edge_invalid(self):
        ...
        self.mock_logger.debug.assert_called_with(
            "Ignoring edge %s -> %s with code %s", None, "b", "code_val"
        )
    ...
    def test_merge_branched_variables(self, mock_compose):
        ...
        self.state.merge(
            (state1, "if", EDGE_TYPES["BRANCH"]), (state2, "else", EDGE_TYPES["BRANCH"])
        )
    ...
    def test_optimize_isolated_node_removal(
        self, mock_remove_node, mock_get_node, mock_node_degree, mock_node_iterator
    ):
        ...
        mock_get_node.return_value = {
            "node_type": NODE_TYPES["VARIABLE"]
        }  # Set return value for get_node
    ...
    def test_optimize_if_node_optimization(
        self,
        mock_add_edge,
        mock_remove_node,
        mock_get_edge_iterator,
        mock_predecessor_node_iterator,
        mock_successor_node_iterator,
        mock_get_node,
        mock_node_degree,
        mock_node_iterator,
    ):
        ...
        mock_get_edge_iterator.return_value = [
            ("key", {"code": "if_code", "edge_type": 0})
        ]
    ...
    def test_optimize_redundant_edge_removal(
        self,
        mock_remove_edge,
        mock_get_edge_iterator,
        mock_adjacent_node_iterator,
        mock_get_node,
        mock_node_degree,
        mock_node_iterator,
    ):
        ...
        mock_get_edge_iterator.return_value = [
            (
                "key1",
                {"code": "redundant_edge_node_x", "edge_type": EDGE_TYPES["INPUT"]},
            ),
            ("key2", {"code": "other_code", "edge_type": 0}),
        ]

        self.state.optimize()

        mock_remove_edge.assert_called_once_with(
            "redundant_edge_node_x", "target_node_for_redundant_edge", "key1"
        )
