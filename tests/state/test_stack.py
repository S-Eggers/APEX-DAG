...
class TestStack(unittest.TestCase):
    def setUp(self):
        ...
        patcher = patch(
            "ApexDAG.state.stack.setup_logging", return_value=self.mock_logger
        )
        ...
        self.mock_state_instance_module = MagicMock(
            spec=State, context="module", parent_context=None
        )
    ...
    def test_create_child_state_with_parent(self):
        self.mock_state_instance_child = MagicMock(
            spec=State, context="child_context", parent_context="module"
        )
        self.mock_state_class.side_effect = [
            self.mock_state_instance_module,
            self.mock_state_instance_child,
        ]
    ...
    def test_create_child_state_without_parent(self):
        self.mock_state_instance_child = MagicMock(
            spec=State, context="child_context", parent_context=None
        )
        self.mock_state_class.side_effect = [
            self.mock_state_instance_module,
            self.mock_state_instance_child,
    ...
    def test_restore_parent_state(self):
        ...
        self.stack._state[
            "child_context"
        ].parent_context = "module"  # Ensure parent_context is set for the mock
        self.stack.restore_parent_state()
        self.assertEqual(self.stack._current_state, "module")

    def test_restore_parent_state_no_parent(self):
        self.mock_state_instance_module.parent_context = (
            None  # Ensure no parent for module state
        )
        with self.assertRaises(ValueError):
            self.stack.restore_parent_state()

    def test_merge_states(self):
        mock_state_child1 = MagicMock(
            spec=State, context="child1", parent_context="module"
        )
        mock_state_child2 = MagicMock(
            spec=State, context="child2", parent_context="module"
        )
    ...
    def test_get_current_state(self):
        self.assertEqual(
            self.stack.get_current_state(), self.mock_state_instance_module
        )
