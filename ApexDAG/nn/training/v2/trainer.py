...
class ApexOnlineTrainer:
    ...
    def __init__(
        self, model, criterion, learning_rate=1e-4, log_dir="runs/v2_experiment"
    ) -> None:
        ...
        logger.info(
            f"Trainer initialized on {self.device}. Logs at {log_dir}/{timestamp}"
        )

    def log_model_graph(self, sample_data) -> None:
        ...
        try:
            ...
            logger.warning(
                f"TensorBoard graph tracing failed (common with PyG dynamic graphs): {e}"
            )
    ...
    def train_step(self, batch_data):
        ...
        loss, loss_node, loss_edge = self.criterion(
            node_logits, batch_data.y_node, edge_logits, batch_data.y_edge
        )
    ...
    def _log_metrics(
        self, total_loss, node_loss, edge_loss, node_logits, edge_logits
    ) -> None:
        ...
        self.writer.add_histogram(
            "Predictions/Node_Classes", node_preds, self.global_step
        )
        self.writer.add_histogram(
            "Predictions/Edge_Classes", edge_preds, self.global_step
        )

    def save_cpu_checkpoint(self, path: str) -> None:
        """
