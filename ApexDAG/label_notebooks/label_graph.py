...
from ApexDAG.label_notebooks.pydantic_models import SubgraphContext
...
class GraphLabeler:
    ...
    def label_edge(self, edge, edge_num_index, max_depth=2):
        ...
        for attempt in range(retry_attempts):
            try:
                ...
                logging.error(
                    f"Rate limit error during request for edge {edge.source} -> {edge.target}: {e}"
                )
                ...
                if attempt < retry_attempts - 1:
                    ...
                    logging.error(
                        f"Exceeded maximum retry attempts for edge {edge.source} -> {edge.target}"
                    )
                    raise e
            except Exception as e:
                logging.error(
                    f"Error during initial request for edge {edge.source} -> {edge.target}: {e}. Trying with depth + 1."
                )
                time.sleep(retry_delay)
                max_depth += 1
    ...
    def label_graph(self):
        ...
        if len(self.G_with_context.edges) > 110:
            logging.warning(
                f"Graph has more than 100 edges ({len(self.G_with_context.edges)}). This may take a long time to process."
            )
            raise ValueError(
                "Graph has more than 100 edges. Please reduce the number of edges before processing. Skipping"
            )

        for edge_index, edge in tqdm(
            enumerate(self.G_with_context.edges),
            total=len(self.G_with_context.edges),
            desc="Processing nodes of graph:",
        ):
            try:
                ...
                logging.error(
                    f"Error during edge labelling: {e}. Filling value with missing."
                )
