...
class Notebook:
    ...
    def create_execution_graph(self, greedy: bool = False):
        ...
        for index, cell in enumerate(self._nb.cells):
            if "execution_count" in cell and cell.execution_count is not None:
                self._logger.debug(
                    f"Adding cell {index} with execution count {cell.execution_count}"
                )
        ...
        if not greedy:
            self._logger.debug(
                "Non-greedy mode: creating execution graph based on execution count"
            )
            prev_index = -1
    ...
    def code(self) -> str:
        ...
        try:
            node_indicies = [
                index for index in nx.dfs_preorder_nodes(self._G, self._source)
            ]
    ...
    def __iter__(self):
        ...
        if (
            len(node_indicies) < self._cell_window_size
            and not self._cell_window_size == -1
        ):
            raise ValueError(
                "Cell window size is bigger than actual cells available. Reduce the window size."
            )
