...
class Draw:
    ...
    def dfg(self, G: nx.DiGraph, save_path: str = None):
        ...
        nx.draw_networkx_labels(
            G, pos, labels=node_labels, font_size=10, font_color="#454545"
        )
        nx.draw_networkx_edge_labels(
            G, pos, edge_labels=edge_labels, font_size=8, font_color="#454545"
        )

        # Custom legend for node types
        node_legend_elements = [
            Patch(facecolor=light_steel_blue, edgecolor="none", label="Variable")
            if light_steel_blue in node_colors
            else None,
            Patch(facecolor=very_soft_blue, edgecolor="none", label="Intermediate")
            if very_soft_blue in node_colors
            else None,
            Patch(facecolor=light_green, edgecolor="none", label="Function")
            if light_green in node_colors
            else None,
            Patch(facecolor=pink, edgecolor="none", label="Import")
            if pink in node_colors
            else None,
            Patch(facecolor=very_soft_yellow, edgecolor="none", label="If")
            if very_soft_yellow in node_colors
            else None,
            Patch(facecolor=very_soft_lime_green, edgecolor="none", label="Loop")
            if very_soft_lime_green in node_colors
            else None,
            Patch(facecolor=very_soft_purple, edgecolor="none", label="Class")
            if very_soft_purple in node_colors
            else None,
            Patch(facecolor="black", edgecolor="none", label="Change")
            if "black" in node_colors
            else None,
        ]
        node_legend_elements = [
            elem for elem in node_legend_elements if elem is not None
        ]

        # Custom legend for edge types
        edge_legend_elements = [
            Line2D([0], [0], color=light_salmon, lw=2, label="Caller")
            if light_salmon in edge_colors
            else None,
            Line2D([0], [0], color=pale_green, lw=2, label="Input")
            if pale_green in edge_colors
            else None,
            Line2D([0], [0], color=gray, lw=2, linestyle="--", label="Omitted")
            if gray in edge_colors
            else None,
            Line2D([0], [0], color=powder_blue, lw=2, label="Branch")
            if gray in edge_colors
            else None,
            Line2D([0], [0], color=peach_puff, lw=2, label="Loop")
            if gray in edge_colors
            else None,
            Line2D([0], [0], color=light_green, lw=2, label="Function Call")
            if light_green in edge_colors
            else None,
            Line2D([0], [0], color="blue", lw=2, linestyle=":", label="Change")
            if "blue" in edge_colors
            else None,
        ]
        edge_legend_elements = [
            elem for elem in edge_legend_elements if elem is not None
        ]
    ...
    def labelled_dfg(self, G: nx.DiGraph, save_path: str = None):
        ...
        edge_colors = [
            domain_color_map.get(
                edge_domain_labels.get(edge, "NOT_INTERESTING"), "black"
            )
            for edge in G.edges
        ]
        ...
        nx.draw_networkx_nodes(
            G, pos, node_color="lightgrey", node_size=1000
        )  # Nodes are now a neutral color
        nx.draw_networkx_edge_labels(
            G, pos, edge_labels=edge_labels, font_size=8, font_color="#454545"
        )
        nx.draw_networkx_labels(
            G, pos, labels=node_labels, font_size=10, font_color="black"
        )  # Display node IDs

        edge_legend_elements = [
            Line2D([0], [0], color=color, lw=2, label=label)
            for label, color in domain_color_map.items()
        ]
    ...
    def ast(self, G: nx.DiGraph, t2t_paths: list[list[int]]):
        ...
        plt.savefig(os.path.join(os.getcwd(), "output", "ast_graph.png"))
        plt.clf()
