...
class LineageSerializer:
    def to_dict(self, graph: PythonDataFlowGraph) -> dict[str, Any]:
            ...
            while queue:
                ...
                for nxt in G.successors(curr):
                    ...
                    if nxt in anchor_nodes:
                        ...
                        edge_type = first_edge_data.get(
                            "edge_type", DOMAIN_EDGE_TYPES.get("DATA_TRANSFORM", 2)
                        )
                        pred_label = first_edge_data.get(
                            "predicted_label",
                            DOMAIN_EDGE_TYPES.get("DATA_TRANSFORM", 2),
                        )

                        inherited_cell_id = first_edge_data.get(
                            "cell_id", G.nodes[nxt].get("cell_id", "unknown_cell")
                        )
        ...
        aux_nodes_in_V = [
            n for n in V.nodes if V.nodes[n].get("node_type") in AUX_ANCHORS
        ]
        ...
        while changed:
            ...
            for u in list(V.nodes):
                ...
                dataset_successors = [
                    s
                    for s in V.successors(u)
                    if V.nodes[s].get("node_type") == DOMAIN_NODE_TYPES["DATASET"]
                ]

                if len(dataset_successors) == 1:
                    v = dataset_successors[0]

                    dataset_predecessors = [
                        p
                        for p in V.predecessors(v)
                        if V.nodes[p].get("node_type") == DOMAIN_NODE_TYPES["DATASET"]
                    ]
                    ...
                    if len(dataset_predecessors) == 1 and u != v:
                        ...
                        sorted_aux = [
                            item[1] for item in sorted(raw_aux, key=lambda x: x[0])
                        ]
                        aux_str = (
                            f" [Inputs: {', '.join(sorted_aux)}]" if sorted_aux else ""
                        )
                        ...
                        V.nodes[u]["transform_history"].extend(
                            V.nodes[v].get("transform_history", [])
                        )
        ...
        for n, data in V.nodes(data=True):
            ...
            safe_data = {
                ...
                if k not in ("id", "base_inputs", "transform_history")
            }
        ...
        for u, v, data in V.edges(data=True):
            ...
            edge_lbl = (
                " -> ".join(clean_ops) if clean_ops else data.get("label", "Transform")
            )
            ...
            safe_data = {
                k: val for k, val in data.items() if k not in ("id", "operations")
            }
            payload.update(safe_data)
