...
from ApexDAG.mining.iterator import JetbrainsNotebookIterator
from ApexDAG.mining.sampler import GraphSamplingPolicy
from ApexDAG.mining.validator import PipelineValidator, ValidationMetrics
from ApexDAG.util.logger import configure_apexdag_logger

# Python 3.12+ syntax warnings from AST parsing legacy code
warnings.filterwarnings("ignore", category=SyntaxWarning)
# Jupyter Notebook missing ID warnings
...
class MiningOrchestrator:
    ...
    def _load_state(self) -> None:
        if self.state_file.exists():
            ...
            message = f"""
            Resuming from index {self.state["current_index"]}.
            Currently kept: {self.state["kept_count"]}
            """
            logger.info(message)
        else:
    ...
    def _append_to_ledger(
        self, filename: str, metrics: ValidationMetrics, kept: bool, reason: str
    ) -> None:
    ...
    def mine(self) -> None:
        ...
        with tqdm(
            ...
            for filename, notebook_dict in self.iterator:
                ...
                # 2. Policy Sampling (Unpacking the tuple)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target", type=int, default=10000, help="Number of good notebooks to keep"
    )
