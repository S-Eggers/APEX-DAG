...
def extract_from_node(node: WIRNode, field: str) -> WIRNode | None:
    ...
    match ast_node.__class__.__name__:
        case "Assign":
            if field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
            elif field == "input":
                return WIRNode(ast_node.value)
            elif field == "output":
                return WIRNode(ast_node.targets)
        case "Call":
            if field == "operation":
                return WIRNode(ast_node.func, True)
            elif field == "input":
                return WIRNode(ast_node.args + ast_node.keywords)
            elif (
                ast_node.func
                and hasattr(ast_node.func, "id")
                and isinstance(ast_node.func.id, str)
                and field == "output"
            ):
        ...
        case "Attribute":
            if field == "caller":
                ...
                return WIRNode(ast_node.attr, True)
            elif field == "output":
                return WIRNode(ast_node.attr + add_id())
        case "Name":
            if field == "output":
                return WIRNode(ast_node.id)
        case "Constant":
            if field == "output":
                return WIRNode(f"{ast_node.value}{add_id()}")
        case "Import":
            if field == "output":
                return WIRNode(ast_node.names)
        case "Module":
            return WIRNode(None)
        case "alias":
            if field == "output":
                return WIRNode(
                    ast_node.asname if ast_node.asname is not None else ast_node.name
                )
            elif field == "input":
                return WIRNode(
                    ast_node.name
                    if ast_node.asname is not None
                    else ast_node.name + add_id()
                )
            elif field == "operation":
                return WIRNode("ImportAs" + add_id())
        case "ImportFrom":
            if field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
            elif field == "input":
                return WIRNode(ast_node.module)
            elif field == "output":
                return WIRNode(
                    [
                        child.name if child.asname is None else child
                        for child in ast_node.names
                    ]
                )
        case "Subscript":
            if field == "operation":
                ...
                return WIRNode(ast_node.slice)
            elif field == "caller":
                return WIRNode(ast_node.value)
        case "Tuple":
            if field == "output":
                return WIRNode(ast_node.elts)
        case "Slice":
            if field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
            elif field == "input":
                return WIRNode(
                    [
                        val if val is not None else "" + add_id()
                        for val in (ast_node.lower, ast_node.upper, ast_node.step)
                    ]
                )
        case "List":
            if field == "input":
        ...
        case "For":
            if field == "input":
                return WIRNode(ast_node.iter)
            elif field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
            elif field == "output":
                return WIRNode(ast_node.target)
        case "Compare":
            if field == "input":
                return WIRNode([ast_node.left, *ast_node.comparators])
            elif field == "operation":
                return WIRNode(ast_node.ops)
        case "BinOp":
            if field == "input":
                return WIRNode([ast_node.left, ast_node.right])
            elif field == "operation":
                return WIRNode(ast_node.op)
        case "Lambda":
            if field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
        case "keyword":
            if field == "input":
                ...
                if ast_node.arg is not None:
                    return WIRNode(ast_node.arg + add_id())
            elif field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
        case "ListComp":
            if field == "operation":
                ...
                return WIRNode([*ast_node.generators, ast_node.elt])
        case "UnaryOp":
            if field == "operation":
                return WIRNode(ast_node.__class__.__name__ + add_id())
            elif field == "input":
                return WIRNode(ast_node.operand)
        case "comprehension":
            if field == "input":
                return WIRNode(ast_node.iter)
            elif field == "output":
        ...
        case "IfExp":
            if field == "operation":
                ...
                return WIRNode([ast_node.test, ast_node.test, ast_node.orelse])
        case "Dict":
            if field == "input":
                return WIRNode(
                    [
                        ...
                        for key, value in zip(
                            ast_node.keys, ast_node.values, strict=False
                        )
                    ]
                )
            elif field == "operation":
...
def GenPR(v: WIRNode, PRs: list[PRType]) -> tuple[WIRNode, list[PRType]]:
    ...
    for _i, _c, _p, _o in itertools.product(
        input_nodes, caller_nodes, operation_nodes, output_nodes
    ):
