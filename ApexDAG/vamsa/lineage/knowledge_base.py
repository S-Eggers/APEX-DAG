...
class KB:
    ...
    def __init__(self, knowledge_base: pd.DataFrame | None = None, kb_csv_path: str | None = None) -> None:
        ...
        if knowledge_base is not None:
            if isinstance(knowledge_base, pd.DataFrame):
                ...
                # Graceful fallback if another KB object was passed
            ...
            return  # Exit early to prevent duplicate fallback rules

        # 2. Disk Loading (Only runs if no DataFrame is injected)
        try:
            if kb_csv_path:
    ...
    def back_query(self, o_list: list, p: str | None) -> list:
        def has_similar_elements(row_list: list, provided_list: list) -> bool:
            ...
            return all(p == r or p is None for p, r in zip(provided_list[:min_len], row_list[:min_len], strict=False))
