...
class KB:
        ...
        self.knowledge_base = pd.DataFrame(
            [
                ...
                },
                {
                    ...
                    "Caller": "model",
                    "API Name": "compile",
                    "Inputs": ["optimizer", "loss_function", "metrics"],
                    "Outputs": ["model"],
                },
                {
                    "Library": "tensorflow",
                    "Module": "keras",
                    "Caller": "compiled_model",
                    "API Name": "fit",
                    "Inputs": ["features", "labels"],
                    "Outputs": ["trained_model"],
                },
    ...
    def back_query(self, O, p):  # TODO: make the name better
        ...
        def has_similar_elements(row_list, provided_list):
            ...
            return all(
                p == r or p is None
                for p, r in zip(provided_list[:min_len], row_list[:min_len])
            )
...
class AnnotationWIR:
    ...
    def annotate(self):
        ...
        for vs in seed_set:
            ...
            while forward_stack:
                ...
                pr = inputs, caller, process, outputs = self._extract_pr_elements(
                    node
                )  # extract PR = <I, c, p, O>
                # print(pr)
                ...
                for input_node, input_annotation in zip(
                    inputs[:min_len], input_annotations[:min_len]
                ):
                ...
                while backward_stack:
                    previous_input, annotation = (
                        backward_stack.pop()
                    )  # Pop a process relation
                    #  Annotate inputs using knowledge base
                    inputs, caller, process, outputs = self._extract_pr_elements(
                        previous_input, node_type="output"
                    )  # Extract PR = <I, c, p, O> - find to which inuts where outputs
                    ...
                    outputs_annotations = [
                        self._get_annotation(vo) for vo in outputs
                    ]  # get the annotation of the previous input and other outputs of same operation
                    input_annotations = self.knowledge_base.back_query(
                        outputs_annotations, process
                    )
    ...
    def find_import_nodes(self):
        ...
        return [
            ...
            if "importas" in node.lower() or "importfrom" in node.lower()
        ]
    ...
    def find_forward_prs(self, pr):
        ...
        return set(
            [next_pr[2] for next_pr in self.prs if is_connected(next_pr)]
        )  # get operations connected

    def draw_graph(
        self, input_nodes, output_nodes, caller_nodes, operation_nodes, output_filename
    ):
        labels = {
            node: remove_id(node) + f" :Annotation {self._get_annotation(node)}"
            for node in self.annotated_wir.nodes()
        }
        ...
        nx.draw_networkx_nodes(
            self.annotated_wir, pos, nodelist=input_nodes, node_shape="o"
        )
        nx.draw_networkx_nodes(
            self.annotated_wir, pos, nodelist=caller_nodes, node_shape="o"
        )
        nx.draw_networkx_nodes(
            self.annotated_wir, pos, nodelist=operation_nodes, node_shape="s"
        )
        nx.draw_networkx_nodes(
            self.annotated_wir, pos, nodelist=output_nodes, node_shape="o"
        )
        ...
        nx.draw_networkx_edges(
            self.annotated_wir, pos, edgelist=edges, edge_color=edge_colors, arrows=True
        )
...
if __name__ == "__main__":
    ...
    wir, prs, tuples = GenWIR(
        parsed_ast,
        output_filename="output/wir-unannotated-test.png",
        if_draw_graph=True,
    )
    ...
    annotated_wir.draw_graph(
        input_nodes,
        output_nodes,
        caller_nodes,
        operation_nodes,
        "output/annotated-wir-final.png",
    )
