...
def extract_from_node(
    node, field
) -> Optional[WIRNodeType]:  # main function, not defined in the paper
    ...
    match node.__class__.__name__:
        ...
        case "Name":
            if field == "output":
                return WIRNode(
                    node.id
                )  # + add_id() # also needs id attribute, does not work sadly...
        case "Constant":
            if field == "output":
        ...
        case "alias":
            if field == "output":
                ...
                )  # also needs id attribute
            elif field == "input":
                return WIRNode(
                    node.name if node.asname is not None else node.name + add_id()
                )
            if field == "operation":
                return WIRNode(
                    "ImportAs" + add_id()
                )  # node.__class__.__name__ + add_id() # name change to make it same an in paper
        ...
        case "Slice":
            if field == "operation":
                ...
                return WIRNode(
                    [
                        value if value is not None else "" + add_id()
...
def GenPR(v: WIRNode, PRs: list[PRType]) -> Tuple[WIRNodeType, list[PRType]]:
    ...
    if isinstance(v.node, (str, int, float, bool, type(None))):
        if isinstance(v.node, str):
            ...
            v = WIRNode(
                return_name
            )  # problematic later on (from_string in Agraph) # add variable tag!
    ...
    if O.node is None:
        ...
        if (
            is_empty_or_none_list(I.node) and c.node is None
        ):  # if we got only caller object and no other proveance, just return caller, do not add an id
            O = p
            return O, PRs
...
def GenWIR(
    root: ast.AST, output_filename: str, if_draw_graph: bool
) -> Tuple[nx.DiGraph, Set[PRType], tuple[Set, Set, Set, Set]]:
    """
    ...
    PRs_filtered = fix_bibartie_issue_import_from(
        PRs_filtered
    )  # fix bipartie issue for import from statements
    ...
    G, (input_nodes, output_nodes, caller_nodes, operation_nodes) = (
        construct_bipartite_graph(
            PR_filtered_no_assign,
            output_filename=output_filename,
            if_draw_graph=if_draw_graph,
        )
    )
    ...
    return (
        G,
        PR_filtered_no_assign,
        (input_nodes, output_nodes, caller_nodes, operation_nodes),
    )
...
def filter_PRs(PRs: Set[PRType]) -> Set[PRType]:
    ...
    for I, c, p, O in PRs:
        if (
            p in problematic_operations and c is None
        ):  # p is an operation produced by another PR
...
def construct_bipartite_graph(
    PRs: Set[PRType], output_filename: str, if_draw_graph: bool
) -> tuple[nx.DiGraph, tuple[Set, Set, Set, Set]]:
    """
...
if __name__ == "__main__":
    ...
    wir, _ = GenWIR(
        parsed_ast, output_filename=f"output/wir-{name}.png", if_draw_graph=True
    )
