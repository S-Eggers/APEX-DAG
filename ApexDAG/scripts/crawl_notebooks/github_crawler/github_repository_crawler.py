...
class GitHubRepositoryCrawler:
    def __init__(self, 
        ...
        self.hash_dict = dict() # storing (user, repository) and the value will be filename
    ...
    def _get_all_pages(self, current_url, number_of_pages):
        ...
        for currentPage in tqdm(range(1, number_of_pages + 1), desc="Processing pages", leave=False):
        ...
        try:
            ...
            def fetch_all_files(url, depth=1):
                ...
                try:
                    ...
                    for file in files:
                        if file['type'] == 'file':
                            if file['name'].endswith(extension_notebooks) or any(file['name'].endswith(ext) for ext in extensions_data):
            ...
            self.logger.error(f"Failed to check for data files in repository {user}/{repository}: {e}")
