...
class NotebookProcessor:
    ...
    def get_alias_pattern(cls, import_table):
        ...
        if alias_key not in cls.alias_pattern_cache:
            if import_table["alias"].empty:
                ...
                alias_pattern = "|".join(
                    re.escape(alias) for alias in import_table["alias"]
                )
    ...
    def get_import_table(notebook_content):
        ...
        import_pattern = re.compile(
            r"^(?:import\s+(\S+)(?:\s+as\s+(\S+))?|from\s+(\S+)\s+import\s+([\w,]+(?:\s*,\s*[\w,]+)*)(?:\s+as\s+([\w,]+))?)$"
        )
    ...
    def simple_check_aliases_in_code_without_import(cell_code, import_table):
        ...
        def check_alias_in_line(alias):
            return any(
                (alias in line.split("#")[0])
                and not (line.startswith("import"))
                and (not line.startswith("from"))
                for line in code_lines
            )  # omit line if starts with comments
    ...
    def methods_used_in_code(cls, code, import_table):
        ...
        code = [
            line.split("#")[0] for line in code if not line.strip().startswith("#")
        ]  # omit line if starts with comments
        code = "\n".join(code)
    ...
    def process_cells(self, cell_code):
        ...
        import_table = self.simple_check_aliases_in_code_without_import(
            cell_code, import_table
        )
    ...
    def extract_code(self, notebook):
        ...
        code_cells = [
            cell["source"]
            for cell in cells
            if (cell.get("cell_type") == "code") and (cell["source"] is not None)
        ]
        return "\n".join(code_cells)
