...
def mine_dataflows_on_github_dataset(args):
    ...
    stats_df.to_csv(
        os.path.join(OUTPUT_DIR, "dfg_experiment_github.csv"), encoding="utf-8"
    )

    github_iterator.print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--start_date", default="2023-02-22", help="Oldest allowble date."
    )
    parser.add_argument(
        "--end_date", default="2025-02-22", help="Newest allowble date."
    )
    parser.add_argument("--stop_index", default=1100000, help="End index")
    parser.add_argument(
        "--parse_repos",
        action="store_true",
        help="If true, create the json file (notebook paths) by going through repositories",
    )
    parser.add_argument(
        "--notebook_paths", default=None, help="Path of json from github repo crawler"
    )
    args = parser.parse_args()

    if args.parse_repos:
        repo_crawler = GitHubRepositoryCrawler(
            query="",
