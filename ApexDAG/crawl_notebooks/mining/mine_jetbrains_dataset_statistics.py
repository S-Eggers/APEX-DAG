...
def process_slice(start_limit, end_limit, json_file, bucket_url, save_dir):
    processor = JetbrainsNotebookProcessor(
        ...
        bucket_url,
        save_dir,
        log_file=f"notebooks_{start_limit}_{end_limit}.log",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json_file",
        type=str,
        default="data/ntbs_list.json",
        help="Path to the JSON file.",
    )
    parser.add_argument(
        "--bucket_url",
        type=str,
        default="https://github-notebooks-update1.s3-eu-west-1.amazonaws.com/",
        help="Bucket URL for downloading notebooks.",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/home/nina/data/apexdag_results/jetbrains_stats_10m/full",
        help="Directory to save the notebooks.",
    )
    parser.add_argument(
        "--start", type=int, default=0, help="Start index for processing."
    )
    parser.add_argument(
        "--end", type=int, default=10000000, help="End index for processing."
    )
