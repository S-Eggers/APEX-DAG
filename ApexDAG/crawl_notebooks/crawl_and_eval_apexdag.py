...
def analyze_dataset(root_path: str, output_file: str = "analysis_report.txt") -> None:
    ...
    for i, nb_path in enumerate(notebooks, 1):
        ...
        if result["success"]:
            ...
            error_key = result["error"][:80] if result["error"] else "Unknown error"

            library_stats[library]["error_messages"][error_key] += 1
            library_stats[library]["error_notebooks"][error_key].append(str(nb_path))
    ...
    report_lines.append(
        ...
        f"({100 * total_success / max(total_notebooks, 1):.1f}%)"
    )
    report_lines.append(
        f"Errors:              {total_errors} "
        f"({100 * total_errors / max(total_notebooks, 1):.1f}%)"
    )
    report_lines.append("")
    ...
    for library in sorted(library_stats.keys()):
        ...
        report_lines.append(
            ...
            f"({100 * stats['errors'] / max(stats['total'], 1):.1f}%)"
        )

        if stats["node_counts"]:
            nc = stats["node_counts"]
            report_lines.append(
                f"  Nodes:     min={min(nc)}, max={max(nc)}, "
                f"mean={sum(nc) / len(nc):.1f}"
            )
        ...
        if stats["error_messages"]:
            ...
            for err, cnt in sorted(
                ...
                for nb in notebooks:
                    report_lines.append(f"        • {nb}")

                remaining = len(stats["error_notebooks"][err]) - len(notebooks)
                if remaining > 0:
