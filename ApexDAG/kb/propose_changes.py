...
class KBChangeManager:
    ...
    def __init__(self, baseline_kb: KB | None = None, corpus_path: str = "") -> None:
    ...
    def load_proposals_from_json(self, json_path: str) -> None:
    ...
    def save_proposals_to_json(self, json_path: str) -> None:
        """Save proposals to a JSON file"""
        with open(json_path, "w") as f:
            json.dump([asdict(p) for p in self.proposals], f, indent=2)

    def apply_proposal(
        self, proposalmaker: ProposalMaker, kb: KB, baseline_data: dict | None = None
    ) -> KB:
        ...
        if "annotation" in proposal.change_type:
            ...
            enhanced_kb.knowledge_base = pd.concat(
                [enhanced_kb.knowledge_base, new_row], ignore_index=True
            )
            ...
            enhanced_kb.knowledge_base_traversal = pd.concat(
                [enhanced_kb.knowledge_base_traversal, new_row], ignore_index=True
            )

            # Note: You'll need to manually add the actual traversal rule function
            # to the KBC dictionary in track_provenance.py
            print(
                f"WARNING: Traversal rule '{proposal.traversal_rule_name}' needs to be manually added to KBC in track_provenance.py"
            )
    ...
    def evaluate_proposal(
        ...
        baseline_data = evaluator_baseline.generate_report(
            baseline_reports, baseline_report_path
        )

        # Apply proposal
        print("\nStep 2/3: Applying proposal and creating enhanced KB...")
        enhanced_kb, proposal = self.apply_proposal(
            proposal_object, self.baseline_kb, baseline_data
        )
        ...
        enhanced_data = evaluator_enhanced.generate_report(
            enhanced_reports, enhanced_report_path
        )
        ...
        impact_level = self._assess_impact_level(
            annotation_coverage_delta, kb_hit_rate_delta, c_plus_delta, c_minus_delta
        )

        recommendation = self._generate_recommendation(
            impact_level, annotation_coverage_delta, kb_hit_rate_delta, proposal
        )

        # Create impact report
        impact_report = ChangeImpactReport(
            proposal=proposal,
            baseline_avg_annotation_coverage=baseline_data["summary"][
            ...
            baseline_unannotated_ops=len(
                baseline_data["most_common_unannotated_operations"]
            ),
            enhanced_avg_annotation_coverage=enhanced_data["summary"][
                "avg_annotation_coverage"
            ],
            ...
            enhanced_unannotated_ops=len(
                enhanced_data["most_common_unannotated_operations"]
            ),
            ...
            comparison_report_path=os.path.join(
                output_dir, f"{safe_name}_comparison.json"
            ),
        )
        ...
        if kb_hit_rate_delta < 0:
            print(
                "WARNING: The proposed change resulted in a negative impact on KB performance. NOT adding to the running KB."
            )
            return impact_report, self.baseline_kb

        else:
            print(
                "The proposed change improved KB performance. ADDing to the running KB."
            )
    ...
    def evaluate_all_proposals(
        self, output_dir: str = "output/proposals"
    ...
    def _print_impact_summary(self, report: ChangeImpactReport) -> None:
        ...
        print("\nMetric Changes:")
        print(
            f"  Annotation Coverage: {report.baseline_avg_annotation_coverage:.2%} → {report.enhanced_avg_annotation_coverage:.2%} ({report.annotation_coverage_delta:+.2%})"
        )
        print(
            f"  KB Hit Rate:         {report.baseline_avg_kb_hit_rate:.2%} → {report.enhanced_avg_kb_hit_rate:.2%} ({report.kb_hit_rate_delta:+.2%})"
        )
        print(
            f"  Avg C+ Size:         {report.baseline_avg_c_plus:.1f} → {report.enhanced_avg_c_plus:.1f} ({report.c_plus_delta:+.1f})"
        )
        print(
            f"  Avg C- Size:         {report.baseline_avg_c_minus:.1f} → {report.enhanced_avg_c_minus:.1f} ({report.c_minus_delta:+.1f})"
        )
        print("\nRecommendation:")
    ...
    def _generate_summary_report(
        self, reports: list[ChangeImpactReport], output_dir: str
    ) -> None:
        ...
        sorted_reports = sorted(
            reports, key=lambda r: r.annotation_coverage_delta, reverse=True
        )
        for i, report in enumerate(sorted_reports[:5], 1):
            print(f"  {i}. {report.proposal.description}")
            print(
                f"     Impact: {report.impact_level} (coverage: {report.annotation_coverage_delta:+.2%})"
            )
...
Examples:
  ...
  python propose_changes.py --corpus /path/to/sklearn --docs https://scikit-learn.org/stable/api/index.html --output output/sklearn
    ...
    for iteration in range(args.iterations):
        ...
        # Evaluate impact
        report, running_KB = manager.evaluate_proposal(
            proposal_object, output_dir=args.output
        )
