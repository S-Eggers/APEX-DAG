...
class Encoder:
    def __init__(
        self,
        logger=None,
        nude_num_types_pretrain=8,
        edge_num_types_pretrain=6,
        min_edges=2,
    ):
        self.logger = logger or print
        self._embedding = Embedding(EmbeddingType.FASTTEXT, logger)
    ...
    def encode_reversed(self, graph: nx.MultiDiGraph, feature_to_encode: str) -> Data:
        ...
        for node_idx, node in enumerate(graph.nodes()):
            ...
            if (
                len(connected_edges_in) == 1 and len(connected_edges_out) == 1
        ...
        )  # Extract for new "nodes"
        edge_features, edge_types = self._extract_node_features(
            graph
        )  # Extract for new "edges"
    ...
    def _sample_negative_edges(self, graph, node_to_id, num_neg_samples):
        ...
        if len(existing_edges) >= len(nodes) * (len(nodes) - 1):
            ...
            self.logger(
                f"WARNING: Not enough negative edges to sample. Found {len(existing_edges)} existing edges, need {len(nodes) * (len(nodes) - 1)}"
            )
        ...
        def process_edge(attrs, is_positive):
            def map_labels_to_int(label):
                if type(label) == str:  # finetune
                    return (
                        DOMAIN_EDGE_TYPES[label] if label in DOMAIN_EDGE_TYPES else -1
                    )
