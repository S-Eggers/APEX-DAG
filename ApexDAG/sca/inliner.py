...
class CallInliner:
    ...
    def process_class(
        self, node: ast.Call, caller_object_name: str, tokens: str, is_instance: bool
    ) -> None:
        parts = tokens.split(".") if tokens else []
        if is_instance:
            class_name = self.stack.instances.get(
                caller_object_name, caller_object_name
            )
            method_name = parts[-1]
        else:
    ...
    def _execute_inline(
        ...
        param_to_caller = self._map_arguments(
            node, target_func, is_class_method=bool(class_name)
        )
        ...
        if class_name:
            instance_version = self._bind_instance(
                node, caller_obj, is_instance, caller_return_var, tokens
            )
        ...
        tokenized_tokens = tokenize_method(
            tokens, self.stack.imported_names, self.stack.import_from_modules
        )
    ...
    def _map_arguments(
        self, node: ast.Call, target_func: str, is_class_method: bool
    ) -> dict:
    ...
    def _bind_instance(
        ...
        raw_code = (
            ast.get_source_segment(self.v.current_cell_source, node.func) or tokens
        )
        label = tokenize_method(
            raw_code, self.stack.imported_names, self.stack.import_from_modules
        )
    ...
    def _bind_self(self, node: ast.Call, instance_version: str) -> None:
    ...
    def _bind_parameters(self, node: ast.Call, param_to_caller: dict) -> None:
        for param_name, caller_node in param_to_caller.items():
            if param_name in self.v._current_state.variable_versions:
                ...
                self.v._add_edge(
                    ...
                    if "CLASS" in str(type(self))
                    else EDGE_TYPES["FUNCTION_CALL"],
                    raw_code="arg_pass",
                    lineno=node.lineno,
                    col_offset=node.col_offset,
                    end_lineno=node.end_lineno,
    ...
    def _bind_returns(
        self, node: ast.Call, target_func: str, caller_return_var: str
    ) -> None:
    ...
    def _execute_class_fallback(
        self, node: ast.Call, caller_obj: str, tokens: str
    ) -> None:
        class_node = self.stack.classes[caller_obj][0]
        self.v._add_node(class_node, NODE_TYPES["CLASS"])
        ...
        if self.v._current_state.current_variable:
            self.v._add_edge(
                source=class_node,
                target=self.v._current_state.current_variable,
                label=label,
                edge_type=EDGE_TYPES["CALLER"],
                raw_code=raw_code,
                lineno=node.lineno,
            ...
            if isinstance(arg, (ast.Name, ast.Attribute, ast.Subscript)):
                ...
                if arg_name:
                    ...
                    code_segment = tokenize_method(
                        arg_name,
                        self.stack.imported_names,
                        self.stack.import_from_modules,
                    )
                    self.v._add_edge(
                        source=arg_version,
                        target=self.v._current_state.current_variable,
    ...
    def _execute_function_fallback(self, node: ast.Call, function_name: str) -> None:
        ...
        if self.v._current_state.current_variable:
            label = tokenize_method(
                function_name, self.stack.imported_names, self.stack.import_from_modules
            )
            self.v._add_edge(
            ...
            for arg in node.args:
                if isinstance(arg, (ast.Name, ast.Attribute, ast.Subscript)):
                    ...
                    arg_name = (
                        flat_args[0] if isinstance(flat_args, list) else flat_args
                    )

                    if arg_name:
                        arg_version = self.stack.get_last_variable_version(arg_name)
                        if arg_version:
                            arg_label = tokenize_method(
                                str(arg_name),
                                self.stack.imported_names,
                                self.stack.import_from_modules,
                            )
