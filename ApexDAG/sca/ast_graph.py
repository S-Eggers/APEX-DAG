...
class ASTGraph(ABC):
    ...
    def get_t2t_paths(self, max_depth=5) -> List[List[int]]:
        ...
        self._leaf_nodes = [
            node for node in self._G.nodes if self._G.out_degree(node) == 0
        ]
            ...
            for other_leaf_node in self._leaf_nodes:
                if leaf_node != other_leaf_node:
                    simple_paths = nx.all_simple_paths(
                        G, leaf_node, other_leaf_node, cutoff=max_depth
                    )
    ...
    def _check_graph_status(self):
        ...
        """
        if not self._build:
            raise ASTGraphNotBuildError(
                "Graph not built. Please parse a notebook or code first."
            )
