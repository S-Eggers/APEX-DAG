...
def create_model(config):
    model = create_pretrain_model(
        config,
        reversed=("reversed" in config["mode"].value),
        tasks=TASKS_PER_GRAPH_TRANSFORM_MODE_FINETUNE[config["mode"].value],
    )
    ...
    if config["pretrained_model_path"]:
        ...
        filtered_state_dict = {
            ...
            if k in model_state_dict and "head" not in k
        }
...
def check_inference_duration(trainer, logger, encoded_graphs):
    ...
    logger.info(
        f"[{i + 1}/{len(encoded_graphs)}] Inference time: {total_inference_time:.4f} seconds"
    )
...
def finetune_gat(args, logger: logging.Logger) -> None:
    ...
    encoded_checkpoint_path = (
        Path(config["encoded_checkpoint_path"]).parent / "pytorch-encoded-finetune"
    )
    ...
    if not encoded_graphs:
        ...
        logger.info(
            f"Loaded {num_graphs} graphs in {load_duration:.2f} seconds "
        ...
        logger.info(
            f"Encoded {num_encoded} graphs in {encode_duration:.2f} seconds "
    ...
    trainer.train(
        ...
        graph_transform_mode=config["mode"],
    )
