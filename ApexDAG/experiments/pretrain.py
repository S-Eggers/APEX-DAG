...
def pretrain_gat(args, logger: logging.Logger) -> None:
    """Main entry point for pretraining the GAT model with masked edge modeling."""
    ...
    setup_wandb(project_name="APEX-DAG-pretraining-masked-edges", name=hash_value)

    checkpoint_path = Path(config["checkpoint_path"])
    encoded_checkpoint_path = Path(config["encoded_checkpoint_path"]).parent / "pytorch-encoded-pretrain-masked-edges"
    ...
    best_val_loss = trainer.train(encoded_graphs, model, Modes.PRETRAINING, device=config['device'])
