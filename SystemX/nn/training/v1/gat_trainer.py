...
from SystemX.util.training_utils import DOMAIN_LABEL_TO_SUBSAMPLE, GraphTransformsMode
...
class GATTrainer:
    ...
    def split_and_subsample(self, dataset: GraphDataset, split_ratio: list, subsample: bool = False) -> tuple:
        ...
        if subsample and self.mode == Modes.FINETUNING:
            ...
            if self.graph_transform_mode in [
                ...
                filtered_indices = [idx for idx in train_dataset.indices if sum(dataset[idx].node_types == index_to_subsample) / len(dataset[idx].node_types) < self.subsample_threshold]
            elif self.graph_transform_mode in [
                GraphTransformsMode.ORIGINAL,
                GraphTransformsMode.ORIGINAL_MASKED,
            ]:
                filtered_indices = [idx for idx in train_dataset.indices if sum(dataset[idx].edge_types == index_to_subsample) / len(dataset[idx].edge_types) < self.subsample_threshold]
            else:
