...
from SystemX.util.training_utils import GraphTransformsMode
...
class BaseTrainer:
    def __init__(
        ...
        graph_transform_mode: GraphTransformsMode = GraphTransformsMode.ORIGINAL,
    ) -> None:
    ...
    def log_confusion_matrix(self, loader: object, phase: str, pred_type: str = "edge_type_preds") -> None:
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(loader, desc="Processing Data for conf matrix", leave=False):
                ...
                if pred_type not in outputs:
                    logger.warning(f"Prediction type '{pred_type}' not found in model outputs. Skipping.")
                    return
    ...
    def train_step(self, data: object) -> object:
        raise NotImplementedError("train_step should be implemented in subclasses")

    def validate_step(self, data: object) -> object:
    ...
    def train(self, num_epochs: int) -> float:
        ...
        best_losses_table = wandb.Table(
            columns=[
                "Epoch",
                "Best_Node_Loss",
                "Best_Edge_Type_Loss",
                "Best_Reconstruction_Loss",
                "Best_Edge_Existence_Loss",
            ]
        )
    ...
    def log_histograms(self, epoch: int) -> None:
        for name, param in self.model.named_parameters():
            self.writer.add_histogram(name, param, epoch)

    def log_embeddings(self, epoch: int, data: object, attribute: str = "node_type_preds") -> None:
        self.model.eval()
        with torch.no_grad():
