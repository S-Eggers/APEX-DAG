...
class Embedding:
    def __init__(self, type: EmbeddingType, max_output_dim: int = 768) -> None:
        self.type = type
        self._model = None
    ...
    def _get_sentence_vector_gemini(self, sequence: str, config: types.EmbedContentConfig | None = None) -> torch.Tensor:
        result = self._model.embed_content(model=self._embedding_model_name, contents=sequence, config=config)
