...
from SystemX.util.training_utils import (
...
class Encoder:
    ...
    def encode(self, graph: nx.MultiDiGraph, feature_to_encode: str) -> Data:
        ...
        source_nodes, target_nodes = zip(*all_edges, strict=False) if all_edges else ([], [])
        ...
        return Data(
            ...
            edge_index=torch.stack([source_nodes_tensor, target_nodes_tensor], dim=0) if all_edges else torch.empty((2, 0), dtype=torch.long),
    ...
    def encode_reversed(self, graph: nx.MultiDiGraph, feature_to_encode: str) -> Data:
        ...
        source_nodes, target_nodes = zip(*new_edges, strict=False) if new_edges else ([], [])
        source_nodes_tensor = torch.tensor(source_nodes, dtype=torch.long)
        target_nodes_tensor = torch.tensor(target_nodes, dtype=torch.long)
        ...
        edge_features_tensor = torch.stack([edge_features[i] for i in new_edges_indices]) if edge_features is not None else None
        edge_types_tensor = torch.tensor([edge_types[i] for i in new_edges_indices], dtype=torch.long) if edge_types is not None else None

        return Data(
            x=node_features,
            node_types=node_types,
            edge_index=torch.stack([source_nodes_tensor, target_nodes_tensor], dim=0) if new_edges else torch.empty((2, 0), dtype=torch.long),
            edge_types=edge_types_tensor,
            edge_features=edge_features_tensor,
        )

    def _sample_negative_edges(self, graph: object, node_to_id: dict, num_neg_samples: int) -> set:
        ...
        if max_possible_edges < len(existing_edges) + num_neg_samples:
            logger.warning(f"Not enough negative edges. Found {len(existing_edges)}, max {max_possible_edges}.")
    ...
    def _extract_node_features(self, graph: object) -> tuple:
    ...
    def _extract_edge_features(self, graph: object, negative_edges: set, feature_to_encode: str = "edge_type") -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        edge_features, edge_types, edge_existence = [], [], []

        def process_edge(attrs: dict, is_positive: bool) -> None:
            def map_labels_to_int(label: object) -> int:
            ...
            edge_emb = self._embedding.embed(attrs.get("label", "")) if is_positive else torch.zeros(self._embedding.dimension)

            edge_type_for_training = int(map_labels_to_int(attrs.get(feature_to_encode, -1))) if is_positive else -1
