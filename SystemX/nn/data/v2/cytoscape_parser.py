...
class CytoscapeParser:
    """Translates raw frontend Cytoscape JSON payloads into PyTorch Geometric Data objects using the V2 pipeline."""
    ...
    def _json_to_networkx(self, cyto_elements: list[dict[str, Any]]) -> nx.MultiDiGraph:
        ...
        for element in cyto_elements:
            ...
            if group == "nodes":
                ...
                ground_truth_label = int(data.get("predicted_label", data.get("edge_type", -1)))
        ...
        logger.debug(f"Parsed Cytoscape JSON into NetworkX graph with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges.")
        return graph

    def process_annotated_json(self, cyto_elements: list[dict[str, Any]]) -> Data:
        """Converts the frontend payload directly into a PyG Data object ready for the online training loop."""
