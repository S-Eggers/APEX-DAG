...
def is_constant(var: str, prs: list[PRType]) -> bool:
    ...
    for pr in prs:
        ...
        if output_nodes is None:
            continue
        if (isinstance(output_nodes, list) and var in output_nodes) or var == output_nodes:
            return False
    return True

def drop_traversal(pr: PRType, tracker: object) -> list:
...
def list_traversal(pr: PRType, tracker: object) -> list:
    return drop_traversal(pr, tracker)

def keyword_traversal(pr: PRType, tracker: object) -> list:
    _input_nodes, _, _, output_node = pr
    if "label" not in output_node:
        return []
    return drop_traversal(pr, tracker)

def iloc_traversal(pr: PRType, tracker: object) -> list:
    _, _, _, output_nodes = pr
...
def subscript_traversal(pr: PRType, tracker: object) -> list:
    return drop_traversal(pr, tracker)

def slice_traversal(pr: PRType, tracker: object) -> list:
    return drop_traversal(pr, tracker)
