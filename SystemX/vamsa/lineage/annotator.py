...
class AnnotationWIR:
    ...
    def annotate(self) -> nx.DiGraph:
        ...
        var_provenance: dict[str, tuple[str | None, str | None]] = {}
        for vs in seed_set:
            lib_mod = self.extract_library_and_module(vs)
            _ins, _caller, _proc, outs = self._extract_pr_elements(vs)
            for vo in outs:
                var_provenance[vo] = lib_mod
        ...
        for vs in seed_set:
            ...
            while forward_stack:
                ...
                if library is not None and ("importas" in str(process).lower() or "importfrom" in str(process).lower()):
                    for vo in outputs:
                        self._annotate_node(vo, library)

                op_library, op_module = var_provenance.get(caller, (library, module))
                for vo in outputs:
                    var_provenance.setdefault(vo, (op_library, op_module))

                context = self._get_annotation(caller)

                if isinstance(context, list):
                    for subcontext in context:
                        input_annotations, annotations_output = self.knowledge_base(op_library, op_module, subcontext, process)
                        if input_annotations or annotations_output:
                            break
                else:
                    input_annotations, annotations_output = self.knowledge_base(op_library, op_module, context, process)

                for vo, annotation_output in zip(outputs, annotations_output, strict=False):
                    self._annotate_node(vo, annotation_output)

                annotated_inputs = []
                min_len = min(len(inputs), len(input_annotations))

                for input_node, input_annotation in zip(inputs[:min_len], input_annotations[:min_len], strict=False):
                ...
                while backward_stack:
                    ...
                    b_inputs, _b_caller, b_process, b_outputs = self._extract_pr_elements(previous_input, node_type="output")
    ...
    def _extract_pr_elements(self, node: str, node_type: str = "operation") -> tuple[list, str, str, list]:
