...
class KB:
    ...
    def __init__(self, knowledge_base: pd.DataFrame | None = None, kb_csv_path: str | None = None) -> None:
        if knowledge_base is not None:
            if isinstance(knowledge_base, pd.DataFrame):
                self.knowledge_base = knowledge_base.copy()
            else:
                self.knowledge_base = knowledge_base.knowledge_base.copy()

            logger.info(f"Initialized Vamsa KB from DataFrame with {len(self.knowledge_base)} entries.")
            return
        ...
        try:
            ...
            message = f"Vamsa KB not found or failed to load. Operating with empty KB. Error: {e}"
        ...
        logger.debug(f"Initialized Vamsa KB with {len(self.knowledge_base)} entries.")
        logger.debug(self.knowledge_base.head())

    def __call__(self, l_str: str | None, l_prime: str | None, c: str | None, p: str | None) -> tuple[list, list]:
        filtered_kb = self.knowledge_base

        if l_str is not None:
            filtered_kb = filtered_kb[(filtered_kb["Library"].fillna("") == remove_id(l_str))]
        ...
        if not filtered_kb.empty:
            if c is not None:
                ...
                filtered_kb = strict_kb if not strict_kb.empty else filtered_kb[filtered_kb["Caller"].isna()]
            else:
    ...
    def back_query(self, o_list: list, p: str | None) -> list:
        def element_matches(kb_output: str, provided: object) -> bool:
            if provided is None:
                return True
            if isinstance(provided, (list, tuple, set)):
                return kb_output in provided
            return provided == kb_output

        def has_similar_elements(row_list: list, provided_list: list) -> bool:
            min_len = min(len(row_list), len(provided_list))
            return all(element_matches(kb_out, given) for given, kb_out in zip(provided_list[:min_len], row_list[:min_len], strict=False))

        filtered_kb = self.knowledge_base
        if p is not None:
            filtered_kb = filtered_kb[(filtered_kb["API Name"].fillna("") == remove_id(p))]
        if (not filtered_kb.empty) and o_list is not None:
            filtered_kb = filtered_kb[(filtered_kb["Outputs"].apply(lambda x: has_similar_elements(x, o_list)))]
