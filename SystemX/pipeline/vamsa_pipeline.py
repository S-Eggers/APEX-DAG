...
from SystemX.sca.constants import DOMAIN_NODE_TYPES
from SystemX.serializer.vamsa_serializer import VamsaSerializer
from SystemX.vamsa import execute_vamsa_pipeline

logger = logging.getLogger(__name__)

class VamsaPipeline:
    def __init__(self, serializer: VamsaSerializer, kb_csv_path: str | None = None) -> None:
    ...
    def _apply_domain_labels(self, annotated_wir: nx.DiGraph) -> None:
        ...
        for node, data in annotated_wir.nodes(data=True):
            ...
            if any(k in ann_strings for k in ["model", "trained_model", "trained model"]):
                semantic_type = DOMAIN_NODE_TYPES["MODEL"]
            elif any(k in ann_strings for k in ["data", "dataset", "features", "labels", "target", "dataframe"]):
                semantic_type = DOMAIN_NODE_TYPES["DATASET"]
            elif ann_strings:
                logger.debug("Unmapped Vamsa annotations on node %s: %s", node, ann_strings)

            annotated_wir.nodes[node]["node_type"] = semantic_type

    def execute(self, cells: list, what_track: set[str] | None = None) -> dict[str, Any]:
        ...
        annotated_wir, c_plus, c_minus = execute_vamsa_pipeline(ast_tree, what_track=what_track, kb_csv_path=self.kb_csv_path)
        self._apply_domain_labels(annotated_wir)

        payload = self.serializer.to_dict(annotated_wir)
        payload["metadata"] = {"vamsa_provenance": {"c_plus": list(c_plus), "c_minus": list(c_minus)}}
